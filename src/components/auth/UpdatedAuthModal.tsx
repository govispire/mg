...
const CompulsoryFormModal = ({ open, onOpenChange, username, onComplete, initialData, onInstantUpdate, role }: CompulsoryFormModalProps) => {
    ...
    const handleModalClose = (open: boolean) => {
        if (!open && !isEditMode) {
            // User is closing the modal without completing it
            // Set a dismissal flag so it won't show again
            localStorage.setItem('compulsoryFormDismissed', 'true');
        }
        onOpenChange(open);
    };
    ...
    return (
        <Dialog open={open} onOpenChange={handleModalClose}>
            <DialogContent
                className="sm:max-w-[540px] max-h-[95vh] overflow-hidden"
            ...
            >
                ...
                <DialogDescription className="text-center text-sm text-muted-foreground">
                    {isEditMode ? `Update your details, ${username}` : `Hey ${username}! 👋 Please fill in these details`}
                </DialogDescription>
                ...
                <form onSubmit={handleSubmit} className="space-y-3 mt-3">
                    ...
                    <div className="flex flex-col items-center space-y-2 mb-3">
                        <div className="relative group cursor-pointer" onClick={() => document.getElementById('avatar-upload')?.click()}>
                            <div className="w-20 h-20 rounded-full overflow-hidden border-2 border-white shadow-lg bg-gray-100 ring-2 ring-primary/5 transition-all duration-300 group-hover:scale-105">
                                <img
                            ...
                            <label htmlFor="avatar-upload" className="absolute bottom-0 right-0 p-1.5 bg-primary rounded-full text-white cursor-pointer hover:bg-primary/90 transition-all shadow-lg hover:scale-110 z-10 border-2 border-white">
                        ...
                        <p className="text-[10px] text-muted-foreground font-medium text-primary">
                    ...
                    <div className="space-y-2.5">
                        ...
                        {isStudent && (
                            <>
                                ...
                                <div className="space-y-1">
                                    <Label htmlFor="exam-category" className="text-xs font-medium">
                                        Select Your Exam Category <span className="text-red-500">*</span>
                                    </Label>
                                    <Select value={examCategory} onValueChange={setExamCategory}>
                                        <SelectTrigger className={cn("h-9 text-sm", formErrors.examCategory && "border-red-500")}>
                                ...
                                {examCategory === 'others' && (
                                    <div className="space-y-1">
                                        <Label htmlFor="custom-exam-category" className="text-xs font-medium">
                                        ...
                                        <Input
                                            ...
                                            className={cn("h-9 text-sm", formErrors.customExamCategory && "border-red-500")}
                                        />
                                ...
                                {examCategory && examCategory !== 'others' && (
                                    <div className="space-y-1">
                                        <Label htmlFor="target-exam" className="text-xs font-medium">
                                            Your Target Exam <span className="text-red-500">*</span>
                                        </Label>
                                        <Select value={targetExam} onValueChange={setTargetExam}>
                                            <SelectTrigger className={cn("h-9 text-sm", formErrors.targetExam && "border-red-500")}>
                                ...
                                {targetExam === 'others' && (
                                    <div className="space-y-1">
                                        <Label htmlFor="custom-target-exam" className="text-xs font-medium">
                                        ...
                                        <Input
                                            ...
                                            className={cn("h-9 text-sm", formErrors.customTargetExam && "border-red-500")}
                                        />
                        ...
                        <div className="space-y-1">
                            ...
                            <EnhancedSelect value={selectedState} onValueChange={setSelectedState}>
                                <EnhancedSelectTrigger className={cn("h-9 text-sm", formErrors.selectedState && "border-red-500")}>
                    ...
                    <Button type="submit" className="w-full h-9 text-sm font-medium mt-3">
