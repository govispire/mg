...
import { ExtendedArticle } from '@/types/currentAffairs';
...
interface ArticleQuizProps {
  article: Article | ExtendedArticle;
  onClose: () => void;
}

// Generate fallback quiz questions based on article meta (when no real quiz items exist)
const generateFallbackQuestions = (article: Article | ExtendedArticle): QuizQuestion[] => {
  const baseQuestions: QuizQuestion[] = [
    {
      id: '1',
      question: `What is the main topic of "${article.title}"?`,
      options: [article.topic, 'International Trade', 'Sports Update', 'Technology Innovation'],
      correctAnswer: 0,
    ...
    },
    {
      ...
      options: ['Sports', article.category, 'Entertainment', 'Lifestyle'],
      correctAnswer: 1,
    ...
    },
    {
      ...
      options: ['Low Priority', 'Medium Priority', 'High Priority', 'Critical Priority'],
    ...
    },
    {
      ...
      options: ['Banking Exams (IBPS, SBI)', 'UPSC Civil Services', 'SSC CGL/CHSL', 'All of the above'],
      correctAnswer: 3,
    ...
    },
    {
      ...
      options: [article.date, 'January 1, 2025', 'December 15, 2024', 'February 20, 2025'],
      correctAnswer: 0,
...
// Convert admin-created QuizItem[] to QuizQuestion[]
const convertQuizItems = (quizItems: any[]): QuizQuestion[] => {
  return quizItems.map((item, idx) => ({
    id: item.id || String(idx + 1),
    question: item.question,
    options: item.options,
    correctAnswer: item.correctAnswer ?? 0,
    explanation: item.explanation || `The correct answer is: ${item.options[item.correctAnswer ?? 0]}`,
  }));
};
...
const ArticleQuiz: React.FC<ArticleQuizProps> = ({ article, onClose }) => {
  ...
  // Use real quiz items if available, otherwise generate fallback
  const extArticle = article as ExtendedArticle;
  const hasRealQuiz = extArticle.quizItems && extArticle.quizItems.length > 0;
  const questions = hasRealQuiz
    ? convertQuizItems(extArticle.quizItems)
    : generateFallbackQuestions(article);
      ...
      <Card className="w-full max-w-2xl mx-auto">
        ...
        <CardContent className="space-y-6">
          <div className="text-center">
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-4xl font-bold text-primary">
    ...
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between mb-2">
          <Badge variant="outline" className="gap-1">
            ...
            {hasRealQuiz && <span className="ml-1 text-green-600">● Live</span>}
          </Badge>
      ...
      <CardContent className="space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Based on: {article.title}</p>
        </div>
        <div className="space-y-3">
          <AnimatePresence mode="wait">
            {currentQuestion.options.map((option, index) => {
              ...
              if (showResult) {
                if (isCorrect) bgClass = 'bg-green-500/10 border-green-500 text-green-700';
                else if (isSelected && !isCorrect) bgClass = 'bg-red-500/10 border-red-500 text-red-700';
                else bgClass = 'bg-muted/30 border-transparent opacity-50';
              }
              return (
                <motion.button
                  key={index}
                  ...
                  className={`w-full p-4 rounded-lg border-2 text-left transition-all flex items-center gap-3 ${bgClass} ${selectedAnswer === null ? 'cursor-pointer' : 'cursor-default'}`}
                >
