...
const TABS = [
  "All", "Banking", "SSC", "Railways", "Insurance", 
  "State Exams", "Defence", "Teaching", "UPSC"
];

const FREQUENTLY_USED = [
  { type: 'rapid-fire', title: 'Rapid Fire', desc: 'Quick practice', qs: '5-10 Questions', icon: Zap, iconBg: 'bg-green-50', iconColor: 'text-green-500' },
  { type: 'mini-test', title: 'Mini Test', desc: 'Concept check', qs: '10-20 Questions', icon: Target, iconBg: 'bg-orange-50', iconColor: 'text-orange-500' },
  { type: 'sectional', title: 'Half Test', desc: 'Better practice', qs: '25-50 Questions', icon: BookOpen, iconBg: 'bg-purple-50', iconColor: 'text-purple-500' },
  { type: 'full-prelims', title: 'Full Test', desc: 'Real exam feel', qs: 'Complete Syllabus', icon: FileText, iconBg: 'bg-blue-50', iconColor: 'text-blue-500' },
];
  ...
  return (
    <div className="p-6 md:p-8 space-y-8 max-w-7xl mx-auto bg-gray-50/50 min-h-screen">
      ...
      <div className="text-sm text-gray-500 font-medium">
        Home <span className="mx-2">&gt;</span> <span className="text-gray-900">Free Quizzes</span>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold text-slate-900">Frequently Used</h2>
          <button 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {FREQUENTLY_USED.map((item, idx) => (
            <div 
              key={idx}
              onClick={() => setSelectedType(item.type as QuizType)}
              className={`bg-white rounded-2xl border ${selectedType === item.type ? 'border-emerald-500 ring-1 ring-emerald-500' : 'border-gray-200'} p-5 cursor-pointer hover:shadow-md transition-all group`}
            >
              <div className="flex flex-col items-center text-center space-y-3">
                <div className={`w-12 h-12 rounded-2xl ${item.iconBg} ${item.iconColor} flex items-center justify-center mb-1 group-hover:scale-110 transition-transform`}>
                  <item.icon className="h-6 w-6" />
                </div>
                <div>
                  <h3 className="font-bold text-slate-900">{item.title}</h3>
                  <p className="text-xs text-slate-500 mt-0.5">{item.qs}</p>
                </div>
              </div>
            </div>
          ))}
